...
const NAV_ITEMS = [
    { icon: '📋', label: 'My Roles', route: 'client.mandates.index' },
]

export default function ClientLayout({ children, breadcrumb = null, noPadding = false }) {
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', background: 'var(--ink)', overflow: 'hidden' }}>
            ...
            <nav style={{
                ...
                {/* Logo */}
                <a href={route('client.mandates.index')} style={{
                    display: 'flex', alignItems: 'center', gap: 9,
                    padding: '0 14px', marginBottom: 20, textDecoration: 'none',
                }}>
                    <div style={{
                        width: 30, height: 30, borderRadius: 8, background: 'var(--sea2)',
                ...
                {NAV_ITEMS.map(item => (
                    <a key={item.route} href={route(item.route)} style={{
                        ...
                        {isActive(item.route) && <div style={{ position: 'absolute', left: 0, top: '20%', bottom: '20%', width: 3, borderRadius: 2, background: 'var(--sea2)' }} />}
                    </a>
                ))}
                ...
                <div style={{ marginTop: 'auto', padding: '10px 12px 4px', borderTop: '1px solid var(--ink2)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <div style={{
                            width: 30, height: 30, borderRadius: '50%', background: 'var(--sea2)',
                            color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 10, fontWeight: 600, flexShrink: 0, border: '2px solid #0B4F8A',
                        }}>{initials(user?.name ?? 'C')}</div>
                        <div style={{ overflow: 'hidden' }}>
                            ...
                            <div style={{ fontSize: 10, color: 'var(--ink4)' }}>Client</div>
                        </div>
                    </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--mist2)' }}>
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--ink4)' }}>
                        ...
                        <span style={{ color: 'var(--ink)', fontWeight: 500 }}>Client Portal</span>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
                        <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, background: 'var(--sea-pale)', color: 'var(--sea2)', fontWeight: 500, border: '1px solid #B8D8F0' }}>
                            Client
                        </span>
                    </div>
                </div>
