...
export default function CandidateShow({ candidate, submissions = [], mandates = [] }) {
    ...
    const saveNote = () => {
        router.post(route('admin.candidates.save-note', candidate.id), { notes: noteData.notes }, {
            preserveState: true,
        })
    }
    ...
    const handleCvUpload = async (e) => {
        ...
        try {
            const res = await fetch(route('admin.candidates.upload-cv', candidate.id), {
                method: 'POST',
                body: fd,
            })
    ...
    return (
        <AdminLayout breadcrumb={[
            { label: 'Candidates', href: route('admin.candidates.index') },
            { label: name },
        ]}>
            ...
            <div style={{ background: candidate.cv_url ? 'var(--jade-pale)' : 'var(--mist2)', borderBottom: '1px solid var(--wire)', padding: '10px 22px', display: 'flex', alignItems: 'center', gap: 14 }}>
                {candidate.cv_url ? (
                    <>
                        ...
                        <a
                            href={route('admin.candidates.download-cv', candidate.id)}
                            target="_blank"
                            rel="noreferrer"
                        ...
                        <div style={{ fontSize: 12, color: 'var(--ink4)' }}>
                            <div style={{ display: 'flex', gap: 9, padding: '8px 0', borderBottom: '1px solid var(--wire)' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: 11, color: 'var(--ink4)' }}>Added to recruiter pool</div>
            ...
            <div style={{ padding: '0 22px 18px' }}>
                <Link href={route('admin.candidates.index')} className="btn btn-secondary btn-sm">← Back to candidates</Link>
            </div>
        </AdminLayout>
    )
}
