...
export default function AddCandidatePage({ mandate, candidates = [], recruiters = [], approvedClaim = null }) {
    ...
    async function triggerPreview(payloadBuilder) {
        ...
        try {
            ...
            const res = await fetch(route('admin.mandates.candidate-ai-preview', mandate.id), {
                method: 'POST',
        ...
        <AdminLayout breadcrumb={[{ label: 'Mandates', href: route('admin.mandates.index') }, { label: mandate.title }, { label: 'Add candidate' }]}>
            ...
            <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 20px 20px' }}>
                <div style={{ marginBottom: 14 }}>
                    <Link href={route('admin.mandates.kanban', mandate.id)} style={{ fontSize: 12, color: 'var(--ink4)', textDecoration: 'none' }}>← Back to workspace</Link>
                </div>
                ...
                <div className="g21" style={{ alignItems: 'start' }}>
                    <div style={{ background: '#fff', border: '1px solid var(--wire)', borderRadius: 'var(--r)', padding: 16 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                            ...
                            <Link className="btn btn-secondary" href={route('admin.mandates.kanban', mandate.id)}>Cancel</Link>
                        </div>
                    </div>
