...
class MandateController extends Controller
{
    ...
    public function store(Request $request, GoogleSheetsService $sheets)
    {
        $data = $request->validate([
            ...
            'contract_type'              => ['nullable', 'in:full_time,contract,part_time'],
            ...
            'must_haves.*'               => ['string', 'max:500'],
            'nice_to_haves'              => ['nullable', 'array'],
            'nice_to_haves.*'            => ['string', 'max:500'],
            'green_flags'                => ['nullable', 'array'],
            'green_flags.*'              => ['string', 'max:500'],
            'red_flags'                  => ['nullable', 'array'],
            'red_flags.*'                => ['string', 'max:500'],
            'screening_questions'        => ['nullable', 'array'],
            'screening_questions.*'      => ['string', 'max:1000'],
            'ideal_candidates'           => ['nullable', 'array'],
            'ideal_candidates.*.name'    => ['nullable', 'string', 'max:200'],
            'ideal_candidates.*.title'   => ['nullable', 'string', 'max:200'],
            'ideal_candidates.*.linkedin_url' => ['nullable', 'string', 'max:500'],
            'ideal_source_companies'     => ['nullable', 'array'],
            'ideal_source_companies.*'   => ['string', 'max:200'],
        ...
        try {
            ...
            return response()->json([
                ...
                'mandate' => [
                    ...
                    'openings_count'          => $parsed['openings_count'] ?? null,
                    ...
                    'ideal_source_companies'  => $parsed['ideal_source_companies'] ?? [],
                ],
            ]);
        } catch (\Throwable $e) {
            return response()->json([
